...
.EXAMPLE
    ...
    Will test the module in .\BuildOutput\PsMoMo\0.1.0\
.EXAMPLE
    .\Test-Module.ps1
    Will test the latest  module version in .\BuildOutput\PsMoMo\
#>
[CmdletBinding()]
param (
  ...
  [ArgumentCompleter({
      ...
      $b_Path = [IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'PsMoMo')
...
begin {
  ...
  if ([string]::IsNullOrWhiteSpace($version)) {
    $version = [version[]][IO.DirectoryInfo]::New([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'PsMoMo')).GetDirectories().Name | Select-Object -Last 1
  }
  $BuildOutDir = [IO.DirectoryInfo]::New((Resolve-Path ([IO.Path]::Combine($PSScriptRoot, 'BuildOutput', 'PsMoMo', $version)) -ErrorAction Stop))
  $manifestFile = [IO.FileInfo]::New([IO.Path]::Combine($BuildOutDir.FullName, "PsMoMo.psd1"))
  ...
  if (!(Test-Path -Path $([IO.Path]::Combine($PSScriptRoot, "PsMoMo.psd1")) -PathType Leaf -ErrorAction Ignore)) { throw [System.IO.FileNotFoundException]::New("Module manifest file Was not Found in '$($BuildOutDir.FullName)'.") }
}
process {
  Get-Module PsMoMo | Remove-Module
