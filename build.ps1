...
.SYNOPSIS
    PsMoMo buildScript
.DESCRIPTION
    A custom Psake buildScript for the module PsMoMo.
.LINK
    https://github.com/alainQtec/PsMoMo/blob/main/build.ps1
.EXAMPLE
...
Begin {
  ...
  $dataFile = [System.IO.FileInfo]::new([IO.Path]::Combine($PSScriptRoot, 'en-US', 'PsMoMo.strings.psd1'))
  if (!$dataFile.Exists) { throw [System.IO.FileNotFoundException]::new('Unable to find the LocalizedData file.', 'PsMoMo.strings.psd1') }
      ...
      Task Deploy -depends Test -description 'Release new github version and Publish module to PSGallery' {
        if ([Environment]::GetEnvironmentVariable($env:RUN_ID + 'BuildSystem') -eq 'VSTS' -or ($env:CI -eq "true" -and $env:GITHUB_RUN_ID)) {
          ...
          try {
            ...
            $latest_Github_release = Invoke-WebRequest "https://api.github.com/repos/alainQtec/PsMoMo/releases/latest" | ConvertFrom-Json
